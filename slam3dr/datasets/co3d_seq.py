...
TRAINING_CATEGORIES = [
    ...
    "laptop","motorcycle","mouse","orange","parkingmeter","pizza","plant","teddybear","toaster","toilet",
    "toybus","toyplane","toytrain","toytruck","umbrella","vase","wineglass",
]
TEST_CATEGORIES = ["ball", "book", "couch", "frisbee", "hotdog", "kite", "remote", "sandwich", "skateboard", "suitcase"]

# ,"microwave",,"stopsign","tv"

class Co3d_Seq(BaseStereoViewDataset):
    def __init__(self, 
                 mask_bg=True, 
                 ROOT="../SLAM3R-main/co3d_dataset_processed", 
                 num_views=2,
    ...
    def _get_views(self, idx, resolution, rng): # rng: np.random.Generator
        # choose a scene
        # * `degree`：每组视图覆盖的角度范围（单位度），例如 90° 表示从一段 90° 的旋转序列中选帧。  
        # * `winsize`：由 `degree / 360 * 100` 计算出的窗口宽度（帧数比例），表示抽样窗口跨越的帧数量。  
        # * `sel_num`：在每个窗口内会生成多少组视图索引，每组占用 `winsize` 范围，因此 `sel_num_perseq = (101 - winsize) * sel_num` 决定每个序列可生成的样本数。  
        # * `num_views`：每组样本包含多少个视图（即最终返回的帧数量），构成多视角输入。
        ...
        while len(imgs_idxs) > 0:  # some images (few) have zero depth
            ...
            views.append(dict(
                img=rgb_image, # PIL image HWC, RGB, [0,1]
                depthmap=depthmap, # depthmap HxW, in meters
                camera_pose=camera_pose, # np.array 4x4
                camera_intrinsics=intrinsics, # np.array 3x3
                dataset='Co3d_v2',
...
if __name__ == "__main__":
    from slam3dr.datasets.base.base_stereo_view_dataset import view_name
    import os
    import trimesh
