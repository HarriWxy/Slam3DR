...
from slam3dr.utils.device import to_numpy, collate_with_cat, to_cpu
from slam3dr.inference import loss_of_one_batch_multiview, \
                                inv, get_multiview_scale
from slam3dr.utils.geometry import xy_grid
...
def depth_correct_pts3d(pts3d, depthmap, conf=None, conf_thres=None,
                        min_valid_depth=1e-3, max_valid_depth=100.0, eps=1e-6):
    """Correct predicted points by fitting depth scale/shift to sensor depth.

    pts3d: (B,H,W,3) tensor in camera coordinates.
    depthmap: (H,W) or (B,H,W) depth in meters.
    conf: optional confidence map (B,H,W).
    """
    if depthmap is None:
        return pts3d, None

    if not torch.is_tensor(depthmap):
        depthmap = torch.tensor(depthmap, device=pts3d.device, dtype=pts3d.dtype)
    if depthmap.ndim == 2:
        depthmap = depthmap.unsqueeze(0)
    if depthmap.shape[0] != pts3d.shape[0]:
        depthmap = depthmap[:pts3d.shape[0]]
    ...
    s = torch.median(d_gt) / (torch.median(d_pred) + eps)
    t = torch.median(d_gt - s * d_pred)
