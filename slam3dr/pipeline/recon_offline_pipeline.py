...
def scene_recon_pipeline_offline(i2p_model:Image2PointsModel, 
    ...
    for i in range(num_views):
        view = dict(label=data_views[i]['label'],
                    ...
                    img_pos=res_poses[i])
        if 'depthmap' in data_views[i]:
            view['depthmap'] = data_views[i]['depthmap']
    ...
    for view_id in tqdm(range(num_views), desc="I2P resonstruction"):
        ...
        if getattr(args, 'depth_correct', False) and 'depthmap' in input_views[view_id]:
            corrected_pts3d, _ = depth_correct_pts3d(
                output[ref_id]['pts3d'],
                input_views[view_id]['depthmap'],
                conf=output[ref_id].get('conf'),
                conf_thres=getattr(args, 'depth_correct_conf_thres', None),
                min_valid_depth=getattr(args, 'depth_correct_min_depth', 1e-3),
                max_valid_depth=getattr(args, 'depth_correct_max_depth', 100.0),
            )
