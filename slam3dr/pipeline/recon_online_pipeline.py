...
from slam3dr.datasets.wild_seq import Seq_Data
from slam3dr.models import Image2PointsModel, Local2WorldModel, inf
from slam3dr.utils.device import to_numpy
from slam3dr.utils.recon_utils import * 
from slam3dr.datasets.get_webvideo import *
from slam3dr.utils.image import load_single_image
from slam3dr.pipeline.recon_offline_pipeline import scene_frame_retrieve
...
def process_input_frame(per_frame_res, registered_confs_mean, 
    ...
    input_view = dict(label=data_views[frame_id]['label'],
                            ...
                            img_pos=temp_pose[0])
    if 'depthmap' in data_views[frame_id]:
        input_view['depthmap'] = data_views[frame_id]['depthmap']
...
def initialize_scene(views:list, model:Image2PointsModel, winsize=5, conf_thres=5, return_ref_id=False,
                     depth_correct=False, depth_correct_min_depth=1e-3,
                     depth_correct_max_depth=100.0, depth_correct_conf_thres=None):    
    ...
    if depth_correct and 'depthmap' in views[init_ref_id]:
        corrected_pts3d, _ = depth_correct_pts3d(
            output['preds'][init_ref_id]['pts3d'],
            views[init_ref_id]['depthmap'],
            conf=output['preds'][init_ref_id].get('conf'),
            conf_thres=depth_correct_conf_thres,
            min_valid_depth=depth_correct_min_depth,
            max_valid_depth=depth_correct_max_depth,
        )
        output['preds'][init_ref_id]['pts3d'] = corrected_pts3d
    initial_pcds = []
    initial_confs = []
...
def initial_scene_for_accumulated_frames(input_views, initial_winsize, 
                                         ...
                                         conf_thres_i2p,
                                         depth_correct=False,
                                         depth_correct_min_depth=1e-3,
                                         depth_correct_max_depth=100.0,
                                         depth_correct_conf_thres=None):
    """initialize the scene with the first several frames.
    ...
    initial_pcds, initial_confs, init_ref_id = initialize_scene(
        ...
        depth_correct_min_depth=depth_correct_min_depth,
        depth_correct_max_depth=depth_correct_max_depth,
        depth_correct_conf_thres=depth_correct_conf_thres,
    )
    # set up the world coordinates with the initial window
...
def recover_points_in_initial_window(window_last_id, buffering_set_ids, 
    ...
    for view_id in range(window_last_id + 1):
        ...
        if depth_correct and 'depthmap' in input_views[view_id]:
            corrected_pts3d, _ = depth_correct_pts3d(
                output[ref_id]['pts3d'],
                input_views[view_id]['depthmap'],
                conf=output[ref_id].get('conf'),
                conf_thres=depth_correct_conf_thres,
                min_valid_depth=depth_correct_min_depth,
                max_valid_depth=depth_correct_max_depth,
            )
            output[ref_id]['pts3d'] = corrected_pts3d
...
def pointmap_local_recon(local_views, i2p_model, 
    ...
    output = i2p_inference_batch([local_views], i2p_model, ref_id=ref_id,
                                    tocpu=False, unsqueeze=False)['preds']
    if depth_correct and 'depthmap' in input_views[current_frame_id]:
        corrected_pts3d, _ = depth_correct_pts3d(
            output[ref_id]['pts3d'],
            input_views[current_frame_id]['depthmap'],
            conf=output[ref_id].get('conf'),
            conf_thres=depth_correct_conf_thres,
            min_valid_depth=depth_correct_min_depth,
            max_valid_depth=depth_correct_max_depth,
        ...
        if (num_frame_pass - 1) % args.perframe == 0:
            ...
            # it's time to initialize the scene
                ...
                initialSceneOutput = initial_scene_for_accumulated_frames(
                                        ...
                                        depth_correct=getattr(args, 'depth_correct', False),
                                        depth_correct_min_depth=getattr(args, 'depth_correct_min_depth', 1e-3),
                                        depth_correct_max_depth=getattr(args, 'depth_correct_max_depth', 100.0),
                                        depth_correct_conf_thres=getattr(args, 'depth_correct_conf_thres', None))
