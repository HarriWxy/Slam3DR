...
PRETRAINED="i2p" # checkpoints/DUSt3R_ViTLarge_BaseDecoder_224_linear.pth
...
torchrun --nproc_per_node=2 train.py \
    ...
    --lr 5e-5 --min_lr 5e-7 --warmup_epochs 1 --epochs 20 --batch_size 16 --accum_iter 1 \
    --save_freq 2 --keep_freq 20 --eval_freq 1 --print_freq 20 --num_workers 16 \
    --save_config\
    --freeze "encoder"\
    --loss_func 'i2p' \
...
torchrun --nproc_per_node=2 train.py \
    ...
    --lr 1e-4 --min_lr 1e-6 --warmup_epochs 1 --epochs 10 --batch_size 4 --accum_iter 1 \
...
torchrun --nproc_per_node=2 train.py \
    ...
    --pretrained $PRETRAINED \
    --pretrained_type "dust3r" \
    --lr 5e-5 --min_lr 5e-7 --warmup_epochs 2 --epochs 20 --batch_size 16 --accum_iter 1 \
    --save_freq 2 --keep_freq 20 --eval_freq 1 --print_freq 20 --num_workers 16\
    --save_config\
    --output_dir $TRAIN_OUT_DIR \
    --freeze "encoder"\
