...
from slam3dr.datasets import get_multi_data_loader  # noqa
from slam3dr.losses import *  # noqa: F401, needed when loading the model
from slam3dr.models import *
from slam3dr.inference import loss_of_one_batch

import slam3dr.utils.croco_misc as misc  # noqa
from slam3dr.utils.croco_misc import NativeScalerWithGradNormCount as NativeScaler  # noqa


def _inject_kwarg(call_str, key, value):
    if f"{key}=" in call_str:
        return call_str
    if not call_str.endswith(')'):
        return call_str
    val_str = repr(value) if isinstance(value, str) else str(value)
    return call_str[:-1] + f", {key}={val_str})"


def _apply_depth_fuse(dataset, args):
    if hasattr(dataset, 'datasets'):
        for ds in dataset.datasets:
            _apply_depth_fuse(ds, args)
        return
    if hasattr(dataset, 'dataset'):
        _apply_depth_fuse(dataset.dataset, args)
        return
    if hasattr(dataset, 'depth_img'):
        dataset.depth_img = True
    if hasattr(dataset, 'depth_fuse'):
        dataset.depth_fuse = True
    if hasattr(dataset, 'depth_fuse_channels'):
        dataset.depth_fuse_channels = args.depth_fuse_channels
    if hasattr(dataset, 'depth_fuse_key'):
        dataset.depth_fuse_key = args.depth_fuse_key
...
def get_args_parser():
    ...
    parser.add_argument('--keep_freq', default=20, type=int,
                        help='frequence (number of epochs) to save checkpoint in checkpoint.pth')
    parser.add_argument('--print_freq', default=50, type=int,
                        help='frequence (number of iterations) to print infos while training')

    parser.add_argument('--freeze', default="encoder", type=str, help='freeze which part of the model')
    ...
    parser.add_argument('--depth_fuse_key', type=str, default='img_depth',
                        help='view key name for fused input tensor')
...
def main(args):
    ...
    # model
    if args.depth_fuse:
        args.model = _inject_kwarg(args.model, 'input_type', args.depth_fuse_key)
        args.model = _inject_kwarg(args.model, 'in_chans', 3 + args.depth_fuse_channels)
    ...
    if args.pretrained =="i2p"and not args.resume:
        model.from_pretrained('./pre_models/slam3dr_i2p')
        ...
        model.from_pretrained('./pre_models/slam3dr_l2w')
...
def save_final_model(args, epoch, model_without_ddp, best_so_far=None):
    ...
    # checkpoint_path = os.path.join("./checkpoints", args.loss_func, "final.pth")  # output_dir / 'checkpoint-final.pth'
