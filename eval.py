# evaluate for offline dataset
from pytools import F
from slam3dr.pipeline.recon_offline_pipeline import scene_recon_pipeline_offline
import argparse
...
parser.add_argument("--keyframe_stride", type=int, default=20, 
...
parser.add_argument("--save_each_frame",action='store_true',default=False,
...
parser.add_argument('--save_preds', default=True, action='store_true', help='whether to save all per-frame preds')    
parser.add_argument('--save_for_eval', default=False, action='store_true', help='whether to save partial per-frame preds for evaluation')   
...
parser.add_argument('--depth_correct_max_depth', type=float, default=100.0,
                    help='maximum valid depth for correction')
parser.add_argument('--depth_correct_conf_thres', type=float, default=None,
                    help='optional confidence threshold for valid pixels')
...
if __name__ == "__main__":
    ...
    if args.i2p_weights is not None:
        ...
        i2p_model = Image2PointsModel.from_pretrained('./pre_models/slam3dr_i2p')
    ...
    if args.l2w_weights is not None:
        ...
        l2w_model = Local2WorldModel.from_pretrained('./pre_models/slam3dr_l2w')
